...
- `works.html` — 成果展示 page. Holds **two kinds of content**: project-born deliverables (deep-linked from `index.html#projects` cards via `.timeline-work-link`) and a 「自研工具箱」 sub-section for self-built, project-independent tools and personal systems. Both use the same `<article class="work-card hover-card" id="…">` block (left: copy + CTA; right: `.work-cover`), so adding an entry means copying an existing `<article>` block — no CSS or JS changes. Status badges: `.work-status-live` (已上线) / `.work-status-open` (已开源) / `.work-status-building` (建设中); a not-yet-usable CTA is `<span class="work-cta work-cta-disabled">`. Cover images are **never cropped** — `.work-cover` deliberately has no fixed aspect ratio and `img` is `width:100%; height:auto`; wrap a screenshot in `<a class="work-cover-link" href="…" target="_blank" rel="noopener">` so it opens full size (the column is only 320px wide), or place `.work-cover-fallback` when there is no screenshot (it carries its own `aspect-ratio:4/3`, which is what keeps the box from collapsing). The 「自研工具箱」 sub-header uses `.works-subtitle`, a selector alias of `.courses-subtitle`. Nav links here point back to `index.html#…` (cross-page), so no nav item is ever `.active` on this page.
